 ...
 */
public class MonetGraphIntegerGenerator<G extends Graph<SimpleNode, SimpleEdge, G>>
implements NetworkGenerator<SimpleNode, SimpleEdge, G> {
	 ...
	 */
	public MonetGraphIntegerGenerator(
			String type,
			int numNodes,
			int numObjectives,
			double alpha) {
	...
	private Weight generateRandomWeights(double firstWeight) {
		...
		for (int i = 1; i < this.numObjectives; i++) {
			w[i] = ((int) (this.randomGenerator.nextDouble() * this.maxWeight));
		}
		return new Weight(w);
	}
	...
	public static void main(String[] args) {
		MonetGraphIntegerGenerator<SimpleDirectedGraph> gn =
				new MonetGraphIntegerGenerator<>("directed", 5, 2, 0.9);
